...
技术栈
・模型框架：GPT-4o 多模态大模型・编程语言：Python・核心依赖：OpenAI SDK、OpenCV、NumPy、Video 处理工具库・核心算法：时序拼贴提示、多模态大模型推理、视频时序特征建模、事故场景分类
项目结构
（预留结构填写位置，可自行补充文件夹划分：数据集、推理脚本、提示词模板、实验结果、工具类等）
运行方式
配置本地 Python 运行环境并安装项目所需依赖库；
配置 GPT-4o 调用密钥与接口环境；
在终端执行以下命令运行项目：
bash
运行
...
论文原信息
论文标题
时序拼贴提示：基于仿真器的低成本交通事故视频识别（结合 GPT-4o）
作者
Pratch Suntichaikul, Pittawat Taveekitworachai, Chakarida Nukoolkit, Ruck Thawonmas
录用会议
2024 年第八届信息技术国际会议（InCIT 2024）
